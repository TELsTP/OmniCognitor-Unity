// src/services/neural-souls/soul-configurator.cjs
const { keyDistributor } = require('./key-distributor.cjs');
...
class NeuralSoulConfigurator {
  ...
  findSoulByArchetype(archetype) {
    return Object.entries(this.souls)
      .find(([_, soul]) => soul.archetype === archetype);
  }

  // Activate soul for task
  async activateSoul(soulId, task) {
    const soul = this.souls[soulId];
  ...
  async performTaskWithSoul(soul, task) {
